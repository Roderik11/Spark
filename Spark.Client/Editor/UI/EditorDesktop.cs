...
using System.Text;
using Squid;
using static Spark.Client.MenuBuilder;
...
namespace Spark.Client
{
    public class EditorDesktop : Desktop
    {
        ...
        public EditorDesktop()
        {
            ...
            AddMenuItem(btn, "New Scene", (s, a) => { Editor.TextureRoundtripTest(); });
            AddMenuItem(btn, "Open Scene", (s, a) => { Editor.LoadScene(""); });
            var sub = AddMenuItem(btn, "Open Recent", null);
            AddSeparator(btn);
            AddMenuItem(btn, "Save Scene", (s, a) => { Editor.SaveScene(""); });
            AddMenuItem(btn, "Save Scene as...", (s, a) => { Editor.SaveScene(""); });
            AddSeparator(btn);
            ...
            AddSeparator(btn);
            AddMenuItem(btn, "Merge Splataps", (sender, args) => { Editor.MergeSplatMaps(); });
