...
namespace Spark
{
    internal static class NoiseUtils
    {
        ...
        internal static double GradientCoherentNoise3D(double x, double y, double z, long seed, QualityMode quality)
        {
            ...
            switch (quality)
            {
                ...
                case QualityMode.Medium:
                    {
                        xs = NoiseUtils.MapCubicSCurve(x - (double)x0);
                        ys = NoiseUtils.MapCubicSCurve(y - (double)y0);
                        zs = NoiseUtils.MapCubicSCurve(z - (double)z0);
                        break;
                    }
                case QualityMode.High:
                    {
                        xs = NoiseUtils.MapQuinticSCurve(x - (double)x0);
                        ys = NoiseUtils.MapQuinticSCurve(y - (double)y0);
                        zs = NoiseUtils.MapQuinticSCurve(z - (double)z0);
                        break;
                    }
            }
            double n0, n1, ix0, ix1, iy0, iy1;
            n0 = NoiseUtils.GradientNoise3D(x, y, z, x0, y0, z0, seed);
            n1 = NoiseUtils.GradientNoise3D(x, y, z, x1, y0, z0, seed);
            ix0 = NoiseUtils.InterpolateLinear(n0, n1, xs);
            n0 = NoiseUtils.GradientNoise3D(x, y, z, x0, y1, z0, seed);
            n1 = NoiseUtils.GradientNoise3D(x, y, z, x1, y1, z0, seed);
            ix1 = NoiseUtils.InterpolateLinear(n0, n1, xs);
            iy0 = NoiseUtils.InterpolateLinear(ix0, ix1, ys);
            n0 = NoiseUtils.GradientNoise3D(x, y, z, x0, y0, z1, seed);
            n1 = NoiseUtils.GradientNoise3D(x, y, z, x1, y0, z1, seed);
            ix0 = NoiseUtils.InterpolateLinear(n0, n1, xs);
            n0 = NoiseUtils.GradientNoise3D(x, y, z, x0, y1, z1, seed);
            n1 = NoiseUtils.GradientNoise3D(x, y, z, x1, y1, z1, seed);
            ix1 = NoiseUtils.InterpolateLinear(n0, n1, xs);
            iy1 = NoiseUtils.InterpolateLinear(ix0, ix1, ys);
            return NoiseUtils.InterpolateLinear(iy0, iy1, zs);
        }

        internal static double GradientNoise3D(double fx, double fy, double fz, int ix, int iy, int iz, long seed)
        {
            long i = (NoiseUtils.GeneratorNoiseX * ix + NoiseUtils.GeneratorNoiseY * iy + NoiseUtils.GeneratorNoiseZ * iz +
                NoiseUtils.GeneratorSeed * seed) & 0xffffffff;
            i ^= (i >> NoiseUtils.GeneratorShift);
            i &= 0xff;
            double xvg = NoiseUtils._randoms[(i << 2)];
            double yvg = NoiseUtils._randoms[(i << 2) + 1];
            double zvg = NoiseUtils._randoms[(i << 2) + 2];
        ...
        {
            return 1.0 - ((double)NoiseUtils.ValueNoise3DInt(x, y, z, seed) / 1073741824.0);
        }

        internal static long ValueNoise3DInt(int x, int y, int z, int seed)
        {
            long n = (NoiseUtils.GeneratorNoiseX * x + NoiseUtils.GeneratorNoiseY * y + NoiseUtils.GeneratorNoiseZ * z +
                NoiseUtils.GeneratorSeed * seed) & 0x7fffffff;
            n = (n >> 13) ^ n;
