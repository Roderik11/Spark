	...
	public partial class DockPane : UserControl, IDockDragSource
	{
		...
		private void InternalConstruct(IDockContent content, DockState dockState, bool flagBounds, Rectangle floatWindowBounds, DockPane prevPane, DockAlignment alignment, double proportion, bool show)
		{
			if (dockState == DockState.Hidden || dockState == DockState.Unknown)
                throw new ArgumentException(Resources.DockPane_SetDockState_InvalidState);

			if (content == null)
                throw new ArgumentNullException(Resources.DockPane_Constructor_NullContent);

			if (content.DockHandler.DockPanel == null)
                throw new ArgumentException(Resources.DockPane_Constructor_NullDockPanel);
			...
			set
			{
				...
				if (value != null)
				{
					if (!DisplayingContents.Contains(value))
                        throw (new InvalidOperationException(Resources.DockPane_ActiveContent_InvalidValue));
				}
				else
				{
		...
		public DockPane SetDockState(DockState value)
		{
			if (value == DockState.Unknown || value == DockState.Hidden)
                throw new InvalidOperationException(Resources.DockPane_SetDockState_InvalidState);
		...
		{
			if (container == null)
                throw new InvalidOperationException(Resources.DockPane_DockTo_NullContainer);
		...
		private void InternalAddToDockList(INestedPanesContainer container, DockPane prevPane, DockAlignment alignment, double proportion)
		{
			if ((container.DockState == DockState.Float) != IsFloat)
                throw new InvalidOperationException(Resources.DockPane_DockTo_InvalidContainer);

			int count = container.NestedPanes.Count;
			if (container.NestedPanes.Contains(this))
				count --;
			if (prevPane == null && count > 0)
                throw new InvalidOperationException(Resources.DockPane_DockTo_NullPrevPane);

			if (prevPane != null && !container.NestedPanes.Contains(prevPane))
                throw new InvalidOperationException(Resources.DockPane_DockTo_NoPrevPane);

			if (prevPane == this)
                throw new InvalidOperationException(Resources.DockPane_DockTo_SelfPrevPane);
        ...
        {
            if (m.Msg == (int)Win32Native.Msgs.WM_MOUSEACTIVATE)
                Activate();
