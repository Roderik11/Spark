        ...
        private class MdiClientController : NativeWindow, IComponent, IDisposable
        {
            ...
            public BorderStyle BorderStyle
            {
                set
                {
                    ...
                    int style = NativeMethods.GetWindowLong(MdiClient.Handle, (int)Win32Native.GetWindowLongIndex.GWL_STYLE);
                    int exStyle = NativeMethods.GetWindowLong(MdiClient.Handle, (int)Win32Native.GetWindowLongIndex.GWL_EXSTYLE);

                    // Add or remove style flags as necessary.
                    switch (m_borderStyle)
                    {
                        case BorderStyle.Fixed3D:
                            exStyle |= (int)Win32Native.WindowExStyles.WS_EX_CLIENTEDGE;
                            style &= ~((int)Win32Native.WindowStyles.WS_BORDER);
                            break;
                    ...
                    NativeMethods.SetWindowLong(MdiClient.Handle, (int)Win32Native.GetWindowLongIndex.GWL_STYLE, style);
                    NativeMethods.SetWindowLong(MdiClient.Handle, (int)Win32Native.GetWindowLongIndex.GWL_EXSTYLE, exStyle);
            ...
            protected override void WndProc(ref Message m)
            {
                switch (m.Msg)
                {
                    case (int)Win32Native.Msgs.WM_NCCALCSIZE:
                        // If AutoScroll is set to false, hide the scrollbars when the control
                        // calculates its non-client area.
                        //if (!AutoScroll)
                            NativeMethods.ShowScrollBar(m.HWnd, (int)Win32Native.ScrollBars.SB_BOTH, 0 /*false*/);
                        break;
                }
            ...
            private void UpdateStyles()
            {
                ...
                NativeMethods.SetWindowPos(MdiClient.Handle, IntPtr.Zero, 0, 0, 0, 0,
                    Win32Native.FlagsSetWindowPos.SWP_NOACTIVATE |
                    Win32Native.FlagsSetWindowPos.SWP_NOMOVE |
                    Win32Native.FlagsSetWindowPos.SWP_NOSIZE |
                    Win32Native.FlagsSetWindowPos.SWP_NOZORDER |
                    Win32Native.FlagsSetWindowPos.SWP_NOOWNERZORDER |
                    Win32Native.FlagsSetWindowPos.SWP_FRAMECHANGED);
            }
        }
