...
namespace Spark.Windows
{
    ...
    //[ToolboxBitmap(typeof(resfinder), "Ionstar.Resources.DockPanel.bmp")]
	...
	public partial class DockPanel : Panel
	{
		...
		public void SetPaneIndex(DockPane pane, int index)
		{
			...
			if (oldIndex == -1)
				throw(new ArgumentException(Resources.DockPanel_SetPaneIndex_InvalidPane));

			if (index < 0 || index > Panes.Count - 1)
				if (index != -1)
                    throw (new ArgumentOutOfRangeException(Resources.DockPanel_SetPaneIndex_InvalidIndex));
	    ...
	    internal Form ParentForm
		{
			...
			{	
				if (!IsParentFormValid())
                    throw new InvalidOperationException(Resources.DockPanel_ParentForm_Invalid);
