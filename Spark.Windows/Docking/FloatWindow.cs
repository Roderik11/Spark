...
namespace Spark.Windows
{
    public class FloatWindow : Form, INestedPanesContainer, IDockDragSource
    {
        ...
        internal const int WM_CHECKDISPOSE = (int)(Win32Native.Msgs.WM_USER + 1);
        ...
        {
            if (dockPanel == null)
                throw (new ArgumentNullException(Resources.FloatWindow_Constructor_NullDockPanel));
        ...
        {
            if (m.Msg == (int)Win32Native.Msgs.WM_NCLBUTTONDOWN)
            {
                if (IsDisposed)
                    return;

                uint result = NativeMethods.SendMessage(this.Handle, (int)Win32Native.Msgs.WM_NCHITTEST, 0, (uint)m.LParam);
            ...
            else if (m.Msg == (int)Win32Native.Msgs.WM_NCRBUTTONDOWN)
            {
                uint result = NativeMethods.SendMessage(this.Handle, (int)Win32Native.Msgs.WM_NCHITTEST, 0, (uint)m.LParam);
            ...
            else if (m.Msg == (int)Win32Native.Msgs.WM_CLOSE)
            {
            ...
            else if (m.Msg == (int)Win32Native.Msgs.WM_NCLBUTTONDBLCLK)
            {
                uint result = NativeMethods.SendMessage(this.Handle, (int)Win32Native.Msgs.WM_NCHITTEST, 0, (uint)m.LParam);
                if (result != 2)	// HITTEST_CAPTION
                {
        ...
        internal void TestDrop(IDockDragSource dragSource, DockOutlineBase dockOutline)
        {
            if (VisibleNestedPanes.Count == 1)
            {
                ...
                if (NativeMethods.SendMessage(Handle, (int)Win32Native.Msgs.WM_NCHITTEST, 0, lParam) == (uint)Win32Native.HitTest.HTCAPTION)
