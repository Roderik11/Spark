...
namespace Jitter.Collision.Shapes
{
    ...
    public abstract class Shape : ISupportMappable
    {
        ...
        public static float CalculateMassInertia(Shape shape, out JVector centerOfMass,
            out JMatrix inertia, bool keepVertices = false)
        {
            float mass = 0.0f;
            ...
            if(keepVertices && shape is ConvexHullShape convex)
            {
                convex.SetVertices(hullTriangles);
            }

            return mass;
        }

        #endregion public static float CalculateMassInertia(Shape shape, out JVector centerOfMass, out JMatrix inertia)
