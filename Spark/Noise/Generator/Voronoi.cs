        ...
        public override double GetValue(double x, double y, double z)
        {
            ...
            for (int zcu = iz - 2; zcu <= iz + 2; zcu++)
            {
                for (int ycu = iy - 2; ycu <= iy + 2; ycu++)
                {
                    ...
                    {
                        double xp = xcu + NoiseUtils.ValueNoise3D(xcu, ycu, zcu, this.m_seed);
                        double yp = ycu + NoiseUtils.ValueNoise3D(xcu, ycu, zcu, this.m_seed + 1);
                        double zp = zcu + NoiseUtils.ValueNoise3D(xcu, ycu, zcu, this.m_seed + 2);
                        double xd = xp - x;
                        double yd = yp - y;
                        double zd = zp - z;
            ...
            if (this.m_distance)
            {
                ...
                double yd = yc - y;
                double zd = zc - z;
                v = (Math.Sqrt(xd * xd + yd * yd + zd * zd)) * NoiseUtils.Sqrt3 - 1.0;
            }
            else
            {
                v = 0.0;
            }
            return v + (this.m_displacement * (double)NoiseUtils.ValueNoise3D((int)(Math.Floor(xc)), (int)(Math.Floor(yc)),
              (int)(Math.Floor(zc)), 0));
        }
