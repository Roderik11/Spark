        ...
        internal static double GradientCoherentNoise3D(double x, double y, double z, long seed, QualityMode quality)
        {
            ...
            switch (quality)
            {
                ...
                case QualityMode.Medium:
                    {
                        xs = MapCubicSCurve(x - (double)x0);
                        ys = MapCubicSCurve(y - (double)y0);
                        zs = MapCubicSCurve(z - (double)z0);
                        break;
                    }
                case QualityMode.High:
                    {
                        xs = MapQuinticSCurve(x - (double)x0);
                        ys = MapQuinticSCurve(y - (double)y0);
                        zs = MapQuinticSCurve(z - (double)z0);
                        break;
                    }
            }
