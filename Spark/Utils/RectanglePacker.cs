...
namespace Spark
{
    ...
    public class RectanglePacker
    {
        ...
        /// <param name="point">Output parameter receiving the rectangle's placement</param>
        /// <returns>True if space for the rectangle could be allocated</returns>
        public bool Pack(int rectangleWidth, int rectangleHeight, out Point point)
        {
            point = new Point();
            ...
            point = this.anchors[anchorIndex];
            ...
            optimizePlacement(ref point, rectangleWidth, rectangleHeight);
            ...
            // positions of the new rectangle
            {
                ...
                bool blocksAnchor =
                  ((point.X + rectangleWidth) > this.anchors[anchorIndex].X) &&
                  ((point.Y + rectangleHeight) > this.anchors[anchorIndex].Y);

                if (blocksAnchor)
                    this.anchors.RemoveAt(anchorIndex);

                // Add new anchors at the upper right and lower left coordinates of the rectangle
                insertAnchor(new Point(point.X + rectangleWidth, point.Y));
                insertAnchor(new Point(point.X, point.Y + rectangleHeight));
            }

            // Finally, we can add the rectangle to our packed rectangles list
            this.packedRectangles.Add(new PackingRectangle(point.X, point.Y, rectangleWidth, rectangleHeight)
            );
